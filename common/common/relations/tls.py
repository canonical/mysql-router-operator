...
from .._charm_libs.charms.tls_certificates_interface.v2 import tls_certificates
from . import secrets
...
class _Relation:
    ...
    _secrets: secrets.RelationSecrets

    @property
    def certificate_saved(self) -> bool:
        """Whether a TLS certificate is available to use"""
        for value in (
            self._secrets.get_value(secrets.UNIT_SCOPE, _TLS_CERTIFICATE),
            self._secrets.get_value(secrets.UNIT_SCOPE, _TLS_CA),
        ):
            if not value:
                return False
        return True
    ...
    def key(self) -> str:
        """The TLS private key"""
        private_key = self._secrets.get_value(secrets.UNIT_SCOPE, _TLS_PRIVATE_KEY)
        if not private_key:
            private_key = _generate_private_key()
            self._secrets.set_value(secrets.UNIT_SCOPE, _TLS_PRIVATE_KEY, private_key)
        return private_key

    @property
    def certificate(self) -> str:
        """The TLS certificate"""
        return self._secrets.get_value(secrets.UNIT_SCOPE, _TLS_CERTIFICATE)

    @property
    def certificate_authority(self) -> str:
        """The TLS certificate authority"""
        return self._secrets.get_value(secrets.UNIT_SCOPE, _TLS_CA)
    ...
    def save_certificate(self, event: tls_certificates.CertificateAvailableEvent) -> None:
        ...
        if (
            ...
            != self._secrets.get_value(secrets.UNIT_SCOPE, _TLS_REQUESTED_CSR).strip()
        ):
        ...
        if (
            ...
            == self._secrets.get_value(secrets.UNIT_SCOPE, _TLS_ACTIVE_CSR)
        ):
        ...
        self._secrets.set_value(secrets.UNIT_SCOPE, _TLS_CERTIFICATE, event.certificate)
        self._secrets.set_value(secrets.UNIT_SCOPE, _TLS_CA, event.ca)
        self._secrets.set_value(secrets.UNIT_SCOPE, _TLS_CHAIN, json.dumps(event.chain))
    ...
    def request_certificate_renewal(self, *, event):
        ...
        self._secrets.set_value(secrets.UNIT_SCOPE, _TLS_REQUESTED_CSR, new_csr.decode("utf-8"))
...
class RelationEndpoint(ops.Object):
    ...
    def _on_set_tls_private_key(self, event: ops.ActionEvent) -> None:
        ...
        self._secrets.set_value(secrets.UNIT_SCOPE, _TLS_PRIVATE_KEY, key)
