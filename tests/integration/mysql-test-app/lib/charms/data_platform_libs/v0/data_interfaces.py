...
class CachedSecret:
    ...
    def get_info(self) -> Optional[SecretInfo]:
        """Wrapper function to apply the call on the Secret object within CachedSecret if any."""
        if self.meta:
...
class DataProvides(DataRelation):
    ...
    def _fetch_specific_relation_data(self, relation, fields: Optional[List[str]]) -> dict:
        ...
        NOTE: Since all secret fields are in the Requires side of the databag
        """
        if not relation.app:
            return {}
            ...
            if fields:
                ...
                for group in secret_fieldnames_grouped:
                    if contents := self._retrieve_group_secret_contents(
                        ...
                        # If it wasn't found as a secret,
                        # let's give it a 2nd chance as "normal" field
